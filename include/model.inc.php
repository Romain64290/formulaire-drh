...
$select = $this->con->prepare('SELECT COUNT(*) as nbr FROM membres
                        WHERE validation_inscription= :etat AND id_typemembre= :nbr');

$select->bindValue(':nbr', 3, PDO::PARAM_INT);
...
return  $resultat;
  ...
  /***********************************************************************
 * Compte le nbre dde d'ADP en attente
 **************************************************************************/
...
$select->bindValue(':nbr', 2, PDO::PARAM_INT);
$select->bindValue(':etat', 0, PDO::PARAM_INT);
$select->execute();		
		}
	catch (Exception $e) {
            echo $e->getMessage() . " <br><b>Erreur lors du calcul du nombre d'AdP en attente</b>\n";
            throw $e;
        	}
